...
{
    __u32       uRiffFcc;       // four character code, "RIFF"
    __u32       uFileLen;       // file total length,  =totalfilelength-8

    __u32       uWaveFcc;       // four character code, "WAVE"

    __u32       uFmtFcc;        // four character code, "fmt "
    __u32       uFmtDataLen;    // Length of the fmt data min(adpcm=0x14,pcm =0x10)
    __u16       uWavEncodeTag;  // WAVE File Encoding Tag adpcm =0x11,pcm =0x01
    __u16       uChannels;      // Channels: 1 = mono, 2 = stereo
    __u32       uSampleRate;    // Samples per second: e.g., 44100
    __u32       uBytesPerSec;   // pcm = sample rate * block align,adpcm= sample rate* uBlockAlign/nSamplesperBlock;
    __u16       uBlockAlign;    // pcm=channels * bits/sample / 8,adpcm =block lenth;
    __u16       uBitsPerSample; // pcm=8 or 16 or 24,adpcm =4,now

    __u16       uWordsbSize;    // for only adpcm =0x0002;
    __u16       uNSamplesPerBlock;//for only adpcm = ((uBlockAlign-4*uChannels)*2+channels)/channels

    //__u8        Reserved[464];  // reserved, for 512bytes aligne
...
//define pcm header information, mux file with pcm data
typedef struct __PCM_HEADER_INF
{
    __u16       uChannels;      // Channels: 1 = mono, 2 = stereo
    __u32       uSampleRate;    // Samples per second: e.g., 44100
    __u32       uBytesPerSec;   // sample rate * block align
...
{
    __u32       ulBsHdrFlag;        // bitstream header flag, = 0xffffffff, for mask bitstream header
    __u32       ulSampleRate;       // sample rate
    __u32       ulActualRate;       // sample rate
    __u16       usBitsPerSample;    // bits per sample
    __u16       usNumChannels;      // channel count
    __u16       usAudioQuality;     // audio quality, don't care(default 100)
    __u16       usFlavorIndex;      // flavor index, don't care(default 0)
    __u32       ulBitsPerFrame;     // bits per frame,don't care(default 0x100)
    __u32       ulGranularity;      // default=0x100;ulFramesPerBlock*ulFrameSize传过来block的大小，一个block包含1个或者多个frames
    __u32       ulOpaqueDataSize;   // size of opaque data
...
{
    __u64       uAACFrmType : 8;        // 1:ADTS Frame; 2:MP4 Audio Specific Config Data
	__u64       uAACObjType : 5;        // __aac_bitstream_sub_t
    __u64       uSFIdxType  : 4;        // sample frequence type, 0:96000, 0xf:follow 24 bits
    __u64       uSampleRate : 24;       // sample frequence, just exist when uSFIdxType = 0xf
    __u64       uChannels   : 4;        // audio channel count
    __u64       uFrmLength  : 1;        // 0:960, 1:1024; default set to 1
    __u64       Flag0       : 1;        // flag reserved 0,
    __u64       Flag1       : 1;        // flag reserved 1,
    __u64       Reserved    : 16;       // reserved, no use
...
{
    __s16       FmtTag;                 //wma format tag, should be 0x160/0x162/0x163
    __s16       Channel;                //audio channel count
    __s32       SampleRate;             //audio sample rate
    __s32       AvgBytePerSec;          //average bytes per seconds
    __s16       BlockAlign;             //block align
    __s16       reserved0;              //reserved
    __u8        reserved1[8];           //reserved
    __s16       EncodeOpt;              //encode options
    __s16       reserved2;              //reserved
    __s16       reserved3;              //reserved

} __wma_header_info_t;

typedef struct __ALAC_HEADER_INFO
{	
	__u32		uAlacFramesPerPacket;
	__u32		uAlacChannelsPerFrame;	
	__u32		uAlacBitsPerChannel;
	__u32		uAlacSamplerate;

}__alac_header_info_t;
