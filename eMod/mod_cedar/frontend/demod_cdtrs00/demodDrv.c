...
__mp *MOpen(__u32 mid, __u32 mode)
{
	...
	WARNING("&&&&&&&&&&&&&&&&&&&&& RS00_init OK &&&&&&&&&&&&&&&&.\n");
    //esKRNL_TimeDly(300);
	#if 0   // for test,  //写入频点测试
	
	WARNING("&&&&&&&&&&&&&&&&&&&&& MB86A2x WRITE TEST FREQ  &&&&&&&&&&&&&&&&.\n");
	RS00_tune(722000, 8000);
...
__s32 MIoctrl(__mp *mp, __u32 cmd, __s32 aux, void *pbuffer)
{
    ...
    switch(cmd)
    {
        ...
        case DEMOD_GET_SIGNAL_STATUS:
            ...
            if((g_Demod->status == WORKING) && (RS00_lockstatus() == 1))
            {
...
//                    ss->strength = dib_signal_strength();
			  RS00_signal_strength_quality(&(ss->quality), &(ss->strength));
                    result = EPDK_OK;
                }
            }
            else
        ...
        case DEMOD_GET_LOCK_STATUS:
        {
	     ...
	     if(0 == result)
	     {
		 	...
		 	{
			 	RS00_tune(g_Demod->frequency, g_Demod->bandwidth /* bandwidth_in_KHz */);		 	
		 	}
	     }
...
//                        return EPDK_OK;
                ...
                // tp->frequency = 722000; //test for dtmb  LXD
                g_Demod->frequency = tp->frequency;
                g_Demod->bandwidth = tp->bandwidth;
        		WARNING("RS00_tune(%ld,%ld)\n", tp->frequency, tp->bandwidth);
                tmpret = RS00_tune(tp->frequency, tp->bandwidth /* bandwidth_in_KHz */);
                if(EPDK_OK == tmpret)
                {
                    return EPDK_OK;
//                    esKRNL_TimeDly(25);
...
//                pDemod->lockCbParam->signalStatus.strength = dib_signal_strength();
		 RS00_signal_strength_quality(&(pDemod->lockCbParam->signalStatus.quality), &(pDemod->lockCbParam->signalStatus.strength));
            }
