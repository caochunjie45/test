...
int iic_init(void)
{
	...
	ftwi = eLIBs_fopen("b:\\BUS\\TWI1", "r");
	
    if(ftwi == NULL)
    {
	 demod_9090_printf("open twi1 failed\n");
        return -1;
    }
	...
	ret = eLIBs_fioctrl(ftwi, TWI_SET_SCL_CLOCK, 0, (void *)(100000));
    	if(ret)
    	{
		demod_9090_printf("set i2c clock[100K] failed\n");
    	}
...
int iic_write(int devAddr, unsigned char* regAddr, int addrSize, unsigned char* txBuf, int txCnt)
{
	...
	if(ftwi)
	{
		...
		demod_9090_printf("i2c write ret[%d]\n", ret);
	}
	else
	{
		demod_9090_printf("i2c write failed ftwi is null\n");
	}
...
int iic_read(int devAddr, unsigned char* regAddr, int addrSize, unsigned char* rxBuf, int txCnt)
{
	...
	if(ftwi)
	{
		...
		demod_9090_printf("i2c read ret[%d], regaddr[%d], [%d], w[%d] \n", ret, stwi.byte_addr[0], stwi.byte_addr[1], addrSize);
	}
	else
	{
		demod_9090_printf("i2c read failed ftwi is null\n");
	}
