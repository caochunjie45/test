...
struct I2C_DRV {
	int (*init)(HANDLE handle, DEVICEID devid,
			__s32 speed, __s32 slaveaddr);
	int (*read)(HANDLE handle, DEVICEID devid,
			__u8 chip, __u8 addr, __u8 alen, __u8 *data, __u8 len);
...
struct TUNER_DRV {
	...
	int (*set_freq)(HANDLE handle, DEVICEID devid, __u32 freq);
	int (*get_rssi)(HANDLE handle, DEVICEID devid, __s32 *rssi);
	int (*deinit)(HANDLE handle, DEVICEID devid);
};
...
static __u8 tuner_addr = FC8300_TUNER_ADDR;
...
__s32 tuner_ctrl_select(HANDLE handle, DEVICEID devid, enum I2C_TYPE type)
{
	switch (type) {
	case FCI_HPI_TYPE:
		tuner_ctrl = &fcihpi;
...
__s32 tuner_ctrl_deselect(HANDLE handle, DEVICEID devid)
{
	if (tuner_ctrl == NULL)
		return BBM_E_TN_CTRL_SELECT;
...
__s32 tuner_i2c_read(HANDLE handle, DEVICEID devid,
		__u8 addr, __u8 alen, __u8 *data, __u8 len)
{
	if (tuner_ctrl == NULL)
		return BBM_E_TN_CTRL_SELECT;
...
__s32 tuner_i2c_write(HANDLE handle, DEVICEID devid,
		__u8 addr, __u8 alen, __u8 *data, __u8 len)
{
	if (tuner_ctrl == NULL)
		return BBM_E_TN_CTRL_SELECT;
...
__s32 tuner_set_freq(HANDLE handle, DEVICEID devid, __u32 freq, __u8 subch)
{
#ifdef BBM_I2C_TSIF
	__u8 tsif_en = 0;
#endif
...
__s32 tuner_select(HANDLE handle, DEVICEID devid,
...
__s32 tuner_deselect(HANDLE handle, DEVICEID devid)
{
	if (tuner == NULL)
		return BBM_E_TN_SELECT;
...
__s32 tuner_get_rssi(HANDLE handle, DEVICEID devid, __s32 *rssi)
{
	if (tuner == NULL)
		return BBM_E_TN_SELECT;
