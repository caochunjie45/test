...
static __u32 fc8300_get_current_clk(HANDLE handle, DEVICEID devid)
{
	__u32 pre_sel, post_sel, multi;
	__u16 pll_set = 0;
...
static __u32 fc8300_get_core_clk(HANDLE handle, DEVICEID devid,
		enum BROADCAST_TYPE broadcast, __u32 freq)
{
	__u32 clk;
...
static __s32 fc8300_set_acif_b31_1seg(HANDLE handle, DEVICEID devid, __u32 clk)
{
#if (BBM_BAND_WIDTH == 6)
	switch (clk) {
	case 100750:
...
static __s32 fc8300_set_acif_1seg(HANDLE handle, DEVICEID devid, __u32 clk)
{
#if (BBM_BAND_WIDTH == 6)
	switch (clk) {
	case 100750:
...
static __s32 fc8300_set_acif_3seg(HANDLE handle, DEVICEID devid, __u32 clk)
{
#if (BBM_BAND_WIDTH == 6)
	switch (clk) {
	case 100750:
...
static __s32 fc8300_set_acif_13seg(HANDLE handle, DEVICEID devid, __u32 clk)
{
#if (BBM_BAND_WIDTH == 6)
	switch (clk) {
	case 100750:
...
static __s32 fc8300_set_cal_front_1seg(HANDLE handle, DEVICEID devid, __u32 clk)
{
#if (BBM_BAND_WIDTH == 6)
	switch (clk) {
	case 97650:
...
static __s32 fc8300_set_cal_front_3seg(HANDLE handle, DEVICEID devid, __u32 clk)
{
#if (BBM_BAND_WIDTH == 6)
	switch (clk) {
	case 97650:
...
static __s32 fc8300_set_cal_front_13seg(HANDLE handle, DEVICEID devid, __u32 clk)
{
#if (BBM_BAND_WIDTH == 6)
	switch (clk) {
	case 97650:
...
static __s32 fc8300_set_default_core_clk(HANDLE handle, DEVICEID devid)
{
	__u16 pll_set;
...
#ifdef BBM_I2C_TSIF
static __s32 fc8300_set_tsif_clk(HANDLE handle, DEVICEID devid)
{
#ifdef BBM_TSIF_CLK_ARBITRARY
	__u32 pre_sel, post_sel = 0, multi = 0, div_sel;
	__u32 input_freq;
	__u32 multi_3, multi_2, multi_1;
#endif
...
__s32 fc8300_reset(HANDLE handle, DEVICEID devid)
{
...
__s32 fc8300_probe(HANDLE handle, DEVICEID devid)
{
	__u16 ver;
...
__s32 fc8300_set_core_clk(HANDLE handle, DEVICEID devid,
		enum BROADCAST_TYPE broadcast, __u32 freq)
{
	__u32 input_freq, new_pll_clk;
	__u32 pre_sel, post_sel = 0, multi = 0, div_sel;
	__u32 current_clk, new_clk, pll_set;
	__u32 multi_3, multi_2, multi_1;
	__s32 res = BBM_OK;
...
__s32 fc8300_init(HANDLE handle, DEVICEID devid)
{
...
__s32 fc8300_deinit(HANDLE handle, DEVICEID devid)
{
#ifdef BBM_I2C_TSIF
...
#if defined(BBM_2_DIVERSITY) || defined(BBM_4_DIVERSITY)
__s32 fc8300_scan_status(HANDLE handle, DEVICEID devid)
{
    __u32 ifagc_timeout       = 70 / SCAN_CHK_PERIOD;
    __u32 ofdm_timeout        = 160 / SCAN_CHK_PERIOD;
    __u32 ffs_lock_timeout    = 100 / SCAN_CHK_PERIOD;
    __u32 cfs_timeout         = 120 / SCAN_CHK_PERIOD;
    __u32 tmcc_timeout        = 1050 / SCAN_CHK_PERIOD;
	__u32 ts_err_free_timeout = 0;
	__u32 data                = 0;
	__u8  a, run;
	__u32 i;
#ifdef BBM_2_DIVERSITY
...
__s32 fc8300_scan_status(HANDLE handle, DEVICEID devid)
{
	...
	for (i = 0; i < ifagc_timeout; i++) {
		bbm_byte_read(handle, DIV_MASTER, 0x3025, &a);
		__msg( "[FC8300] ifagc_timeout 0x%x cnt:%d\n", a, i);
	...
	for (; i < ofdm_timeout; i++) {
		bbm_byte_read(handle, DIV_MASTER, 0x3025, &a);
		__msg( "[FC8300] ofdm_timeout 0x%x cnt:%d\n", a, i);
	...
	for (; i < ffs_lock_timeout; i++) {
		bbm_byte_read(handle, DIV_MASTER, 0x3026, &a);
		__msg( "[FC8300] ffs_lock_timeout 0x%x cnt:%d\n", a, i);
	...
	for (i = 0; i < cfs_timeout; i++) {
		bbm_byte_read(handle, DIV_MASTER, 0x3025, &a);
		__msg( "[FC8300] cfs_timeout 0x%x cnt:%d\n", a, i);
	...
	bbm_byte_read(handle, DIV_MASTER, 0x2023, &a);
	__msg( "[FC8300] cfs_timeout2 0x%x cnt:%d\n", a, i);
	...
	for (i = 0; i < tmcc_timeout; i++) {
		bbm_byte_read(handle, DIV_MASTER, 0x3026, &a);
		__msg( "[FC8300] tmcc_timeout 0x%x cnt:%d\n", a, i);
	...
	switch (broadcast_type) {
	case ISDBT_1SEG:
		bbm_word_read(handle, DIV_MASTER, 0x4113, (__u16 *) &data);
	...
	for (i = 0; i < ts_err_free_timeout; i++) {
		bbm_byte_read(handle, DIV_MASTER, 0x50c5, &a);
		__msg( "[FC8300] ts_err_free_timeout 0x%x cnt:%d\n", a, i);
...
__s32 fc8300_set_broadcast_mode(HANDLE handle, DEVICEID devid,
		enum BROADCAST_TYPE broadcast)
{
	__s32 res = BBM_OK;
	__u32 clk = fc8300_set_default_core_clk(handle, devid);
