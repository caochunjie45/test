...
int iic_write(int devAddr, unsigned char* regAddr, int addrSize, unsigned char* txBuf, int txCnt)
{
	...
	if(ftwi)
	{
		...
		//demod_ATBM781X_printf("i2c write devAddr = %x ret = %d   reg[%x] \n", devAddr,ret,iic_addr[0]);
		WARNING("i2c write devAddr = %x ret = %d   reg[%x] \n", devAddr,ret,iic_addr[0]);
	}
	else
	{
		WARNING("i2c write failed ftwi is null\n");
	}
...
int iic_read(int devAddr, unsigned char* regAddr, int addrSize, unsigned char* rxBuf, int txCnt)
{
	...
	if(ftwi)
	{
		...
		WARNING("i2c read devAddr = %x ret = %d   reg[%x] \n", devAddr,ret,iic_addr[0]);
	}
	else
	{
		WARNING("i2c read failed ftwi is null\n");
	}
