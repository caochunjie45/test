...
{
/*	__s32 ret;
#if 1 //(TS_INTERFACE_TYPE == 1)    //* use ssi mode.
    __u32 tmp;
    tmp = REG_PIOE_CFG0;
    tmp &= ~(0x000fffff);
    tmp |= 0x00033333;
...
/*static*/ __s32 demoPowerOn(void)
{
    ...
    demod_MB86A2x_printf("entry demod power on\n");
...
__mp *MOpen(__u32 mid, __u32 mode)
{
	...
	WARNING("&&&&&&&&&&&&&&&&&&&&& MB86A2x_init OK &&&&&&&&&&&&&&&&.\n");
    //esKRNL_TimeDly(300);
	#if 1   // for test,  //写入频点测试
	
	WARNING("&&&&&&&&&&&&&&&&&&&&& MB86A2x WRITE TEST FREQ  &&&&&&&&&&&&&&&&.\n");
	MB86A2x_tune(473143, 0);//LXD
	WARNING("&&&&&&&&&&&&&&&&&&&&& MB86A2x WRITE TEST FREQ OK &&&&&&&&&&&&&&&&.\n");
	#endif
    //* return
    return (__mp*)g_Demod;
...
__s32 MIoctrl(__mp *mp, __u32 cmd, __s32 aux, void *pbuffer)
{
    ...
    switch(cmd)
    {
        ...
        case DEMOD_GET_SIGNAL_STATUS:
            ...
            if((g_Demod->status == WORKING) && (MB86A2x_lockstatus() == 1))
            {
...
//                    ss->strength = dib_signal_strength();
			  MB86A2x_signal_strength_quality(&(ss->quality), &(ss->strength));
                    result = EPDK_OK;
                }
            }
            else
        ...
        case DEMOD_GET_LOCK_STATUS:
        {
	     ...
	     if(0 == result)
	     {
		 	...
		 	{
			 	MB86A2x_tune(g_Demod->frequency, g_Demod->bandwidth/100 /* bandwidth_in_100KHz */);		 	
		 	}
	     }
...
//                        return EPDK_OK;
                ...
                tmpret = MB86A2x_tune(tp->frequency, tp->bandwidth/100 /* bandwidth_in_100KHz */);
                if(EPDK_OK == tmpret)
                {
                    return EPDK_OK;
//                    esKRNL_TimeDly(25);
...
//                pDemod->lockCbParam->signalStatus.strength = dib_signal_strength();
		 MB86A2x_signal_strength_quality(&(pDemod->lockCbParam->signalStatus.quality), &(pDemod->lockCbParam->signalStatus.strength));
            }
