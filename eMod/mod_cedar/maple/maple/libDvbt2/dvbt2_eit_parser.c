...
static __s32 eit_parser_get_event_info(eit_event_info_t** eitEventInfoHeader, __u8* eventBuf, __s32 eventLen, __s32 eventType)
{
	...
	do
	{
		...
		if(parseFlag)
		{
			...
			{
				__msg("alloc fail when parsing event info.\n");
				return -1;
			}	
			...
			if (PRESENT_EVENT == eventType)
				eitEventInfoPt->runningStatus = runningStatus & 0x0f;
...
#if 0
	//* for dvb2-s(shenzhou, 2tuner dvr), some services have the same [orig_network_id, ts_id, service_id],
...
__s32 eit_parser_get_schedule(__u32 tpId, __u16 tsId, __u16 origNetworkId, __u16 serviceId, maple_schdl_list_t* schdlList, __u32 maxEventNum)
{
	...
	for (i=0, j=0; i<EIT_SCHEDULE_TABLE_ID_NUM; i++)
	{
		...
		while(pSchdl != NULL)
		{
			...
			if (pSchdl->eitShtDesc != NULL)
			{
				pEvent = &schdlList->pSchdlEvents[j];

				dvbt2_convert_str_to_utf8(pSchdl->eitShtDesc->eventName, 
										pSchdl->eitShtDesc->eventNameLen, 
										pEvent->name, 
										MAPLE_MAX_NAME_LEN,
										pSchdl->eitShtDesc->langCode);
                if(shtDesc->textDescFlag)
                {
                    static __u8  tempTextDesc[2048] = {0};
                    __s32 realLen = 0;
                    
                    __wrn("have textDesc\n");
                    #if 0
                    realLen = dvbt2_convert_str_to_utf8(shtDesc->textDesc, 
										shtDesc->textDescLen, 
										tempTextDesc, 
										sizeof(tempTextDesc),
										shtDesc->langCode);
                    if (realLen > 0)
                    {
                        shtDesc->textDesc = (__u8 *)realloc(shtDesc->textDesc, (realLen+2));
                        if (shtDesc->textDesc)
                        {
                            memset(shtDesc->textDesc, 0, realLen);
                            memcpy(shtDesc->textDesc, tempTextDesc, realLen);
                        ...
                        else
                        {
                            ...
                            __wrn("readlloc text desc fail, readlen:%d, oldLen:%d\n",
                                realLen,shtDesc->textDescLen);
                        }
                    }
                    else
                    {
                        pEvent->eventContent = NULL;
                        pEvent->eventContentLen = 0;
                        __wrn("str to utf8 error. textLen:%d, readLen:%d\n", 
                            shtDesc->textDescLen, realLen);
                    }
                    #else
                    pEvent->eventContent = shtDesc->textDesc;
                    pEvent->eventContentLen = shtDesc->textDescLen;
...
__s32 eit_parser_get_pf(__u32 tpId, __u16 tsId, __u16 origNetworkId, __u16 serviceId, maple_sepg_t* pf)
{
	...
	while(pfEvent != NULL)
	{
		...
		if (eitShtDesc != NULL)
		{
			...
			{
				dvbt2_convert_str_to_utf8(eitShtDesc->eventName, 
