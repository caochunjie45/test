...
#define EIT_SCHEDULE_TABLE_ID_NUM           16 //2, 固定为16, isdb的eit,直接把数组都开好.因为存在的tableid不是连续的.这和欧洲的dvb不同.
#define ISDB_EIT_SCHEDULE_TABLEID_NUM       2 //日本isdb有些怪, 0x50~0x57是一段, 0x58~0x5f是一段.而且是对称的,即0x50和0x58描述的是同一天的epg,一个是basic一个是extended.所以这个宏规定0x50~0x57取几张表.对应的0x58~0x5f就取几张表
