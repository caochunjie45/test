...
__s32 MIoctrl(__mp *mp, __u32 cmd, __s32 aux, void *pbuffer)
{
    ...
    switch(cmd)
    {
        ...
        {
            ret = (aux==CEDAR_LRC_TYPE_ITXT) || (aux==CEDAR_LRC_TYPE_ISSA) ||(aux==CEDAR_LRC_TYPE_ISUB) || (aux==CEDAR_LRC_TYPE_PGS) 
#if PLAY_ISDBT_CLOSECAPTION
						|| (aux==CEDAR_LRC_TYPE_ISDB)
#endif
#if PLAY_DVBT_DVBSUBTITLE
						|| (aux==CEDAR_LRC_TYPE_DVBSUB)
#endif
#if PLAY_DVBT_DVBTELETEXT
						||(aux==CEDAR_LRC_TYPE_TTXT)
#endif
						|| (aux==CEDAR_LRC_TYPE_PGS);
