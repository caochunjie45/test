						...
						unsigned char *SeekFail)
{
	...
	do{
		if(Si4702_FM_Seek_Start(seek_mode, channel_space, pChannel_Freq, SeekFail, &valid_channel) != 0) return 1;	
		loop_counter++;
...
__s32  fm_auto_search(__s32 freq, __u32 search_dir)
{
    ...
    else
    {
        ...
        if( freq < 87500 && freq >= 87000 )
        {
        	...
        	if( freq == 87100 )
        	{
        		...
        		delaycounter = 0;
		        while(ReadSTC() !=1 && delaycounter < 250 )delaycounter++;
		        esKRNL_TimeDly(5);
   ...
   FM_SI47XX_WriteReg(W03h,SeekedChannel);
	...
	delaycounter = 0;
	while(ReadSTC() !=1 && delaycounter < 250 )delaycounter++;
	if( delaycounter >= 250 ) __inf("ReadSTC() != 1\n");
	esKRNL_TimeDly(5);
	FM_SI47XX_WriteReg(W03h,SeekedChannel);
	delaycounter = 0;
	while(ReadSTC()  == 1 && delaycounter < 250)delaycounter++;
