...
static __s32  _init_hpgb( HPGB hpgb, H_WIN hwin )
{
	...
	para_p = ((progbars_attribute_t *)GUI_WinGetAttr(hwin))->progbars_paint;	

    dwStyle = GUI_WinGetStyle( hwin );
	
	/* check the input arguments */
	if(    para_p == NULL || para_p->head_bmp_buf == NULL || para_p->tail_bmp_buf   == NULL || para_p->frame_bmp_buf == NULL)
	{
		return -1;
	}
