...
typedef struct _HSLID_t
{
	...
	void                     *bk_matrix;
	void                     *ft_matrix;
	void                     *cursor_bmp_buf;       //光标图片 buffer
	__u32                    cursor_length;         //光标长度
	...
	GUI_MEMDEV_Handle        hmem;                  //显示内存块
	__s32                    min;                   //光标的最小值
	__s32                    max;                   //光标的最大值
	__s32                    value;                 //光标的值
	__s32                    length; 
	__s32                    line_size;             //光标移动一次前进的距离
	__s32                    last_keyaction;        //last_key
	__bool                   is_internal_cmd;       //是否是内部命令
	__bool                   is_waking;
	__bool                   is_showing;
	__s32                    old_length;
...
static void  _draw_text_vert(HSLID hslid);
static void  _draw_text_hori(HSLID hslid);
...
static void   _slider_reset_i( HSLID hslid )
{
    ...
    hslid->length          = 0;
    hslid->last_keyaction  = KEY_UP_ACTION;
...
static void  _draw_text_vert(HSLID hslid)
{
	...
	GUI_SetFont(hslid->draw_font);
	
	old_mode = GUI_SetDrawMode(GUI_DRAWMODE_TRANS);

    GUI_SetColor(hslid->txt_color);

	eLIBs_int2str_dec( hslid->value,value_buf);
	
	{
	    GUI_RECT  rect;
	
	    rect.x0  = hslid->disp_rect.x;                        
	    rect.y0  = hslid->disp_rect.y + hslid->disp_rect.height;                        
	    rect.x1  = rect.x0 + hslid->text_pos.x;
	    rect.y1  = rect.y0 + hslid->text_pos.y;

    	GUI_ClearRect(rect.x0,rect.y0,rect.x1,rect.y1);
	
	    GUI_DispStringInRect(value_buf,&rect,GUI_TA_VCENTER | GUI_TA_HCENTER);
	
	
	}

    GUI_DispStringAt(value_buf,hslid->disp_rect.x,hslid->disp_rect.height);
...
static void   _draw_text_hori(HSLID hslid)
{
...

	{
	    ...
	    rect.x0  = hslid->disp_rect.x + hslid->disp_rect.width;                        
	    rect.y0  = hslid->disp_rect.y -5;                        
	    rect.x1  = rect.x0 + hslid->text_pos.x;
	    rect.y1  = rect.y0 + hslid->text_pos.y;
...
static __s32  _SliderCtrlProc(__gui_msg_t *msg)
{
    ...
    dwStyle = GUI_WinGetStyle(hwin);   //窗口模式
    
	is_vertical = ( dwStyle & SDS_VERTICAL ) ? EPDK_TRUE : EPDK_FALSE;//垂直模式还是水平模式
    ...
    switch(msg->id)
    {
        case GUI_MSG_CREATE:
        {
            ...
            if(_init_hslid( hslid, hwin)) //初始化资源失败
            {
        		_release_res( hslid );
            	g_free( hslid );
            	hslid = NULL;
        ...
        { 
			...
			slider_attribute = (slider_attribute_t *)GUI_WinGetAttr(hwin);
			
			if(NULL == slider_attribute)
			{
				eDbug("slider_attribute is NULL \n");
                return 0;
				
			}
			
			slider_action = slider_attribute->slider_action;
		            ...
		            switch( msg->dwAddData2 )
		            {
		            	case KEY_UP_ACTION:
		            	{
		            		if(hslid->last_keyaction == KEY_DOWN_ACTION)
		            		{
								...
								_notify_parent( hwin, SDN_STOPDRAG, 0 );//停止拖动
		            		}
		            		else
		            		{
		            			_notify_parent( hwin, SDN_KEY, msg->dwAddData1 );
				...
				case GUI_MSG_KEY_LONGESCAPE:				
				{
		            switch( msg->dwAddData2)
		            {
		            	case KEY_UP_ACTION:
		            	{
							...
							if(NULL == slider_action.return_action)
							{
								eDbug("slider_action.return_action is NULL \n");
				                return 0;
							}
							...
							slider_action.return_action(&notifymsg);
...
static __s32  _init_hslid(HSLID hslid, H_WIN hwin)
{
	...
	para_p = ((slider_attribute_t *)GUI_WinGetAttr(hwin))->slider_paint;
	
    dwStyle = GUI_WinGetStyle(hwin);
	
	/* check the input arguments */
	if(para_p.head_bmp_buf == NULL|| para_p.tail_bmp_buf == NULL|| para_p.cursor_bmp_buf == NULL )
	{
		return -1;
	}
	...
	hslid->txt_color  = para_p.txt_color;
	hslid->draw_font  = para_p.draw_font;
	hslid->text_pos.x = para_p.text_pos.x;
	hslid->text_pos.y = para_p.text_pos.y;
    ...
    if(is_vertical) //垂直
    {
		hslid->disp_rect.width  = cursor_width ;   //显示区域的宽度为光标的宽度
		hslid->disp_rect.height = rect.height - hslid->text_pos.y;
		hslid->cursor_length    = cursor_height;  //光标的高度
	}
	else//水平
	{
		hslid->disp_rect.width  = rect.width - hslid->text_pos.x;
		hslid->disp_rect.height = cursor_height;  //显示区域的高度为光标的高度
		hslid->cursor_length    = cursor_width;   //光标的宽度
	}
    ...
    /* reset status */
	_slider_reset_i(hslid);     //初始化滑动条的一些数值
