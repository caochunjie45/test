...
static __s32 __volume_install_hide_timmer(movie_volume_scene_t* scene_para)
{
	...
	{
		GUI_SetTimer(scene_para->hfrm, scene_para->hide_timmer_id, 300, NULL);
	}
	else
	{
...
static __s32 __volume_reset_hide_timmer(movie_volume_scene_t* scene_para)
{
	...
	if (GUI_IsTimerInstalled(scene_para->hfrm, scene_para->hide_timmer_id))
	{
		GUI_ResetTimer(scene_para->hfrm, scene_para->hide_timmer_id, 300, NULL);
	}			
	...
	switch(msg->id)
	{
		...
		case GUI_MSG_KEY:
		{
			...
			if (KEY_UP_ACTION == msg->dwAddData2/* || KEY_REPEAT_ACTION == msg->dwAddData2*/)
			{
				...
				if (GUI_MSG_KEY_VDEC == last_key || GUI_MSG_KEY_IR_VDEC== last_key /*|| GUI_MSG_KEY_LONGVDEC == last_key
				...
				else if (GUI_MSG_KEY_VADD == last_key || GUI_MSG_KEY_IR_VADD== last_key /*|| GUI_MSG_KEY_LONGVADD == last_key
			...
			else if(KEY_REPEAT_ACTION == msg->dwAddData2)
			{
				...
				{
					menu_key_delay = 0;
					if(GUI_MSG_KEY_LONGVDEC == last_key)
					{
						__volume_reset_hide_timmer(scene_para);
