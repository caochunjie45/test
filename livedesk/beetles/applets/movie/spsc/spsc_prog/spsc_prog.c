...
static __s32 __spsc_prog_proc(__gui_msg_t* msg)
{
	switch(msg->id)
	{
		...
		case GUI_MSG_KEY:
		{
			...
			{
				if (GUI_MSG_KEY_DOWN == last_key
				/*	||GUI_MSG_KEY_VADD == last_key*/  //cai modify 将VADD替换为DOWN
				   || GUI_MSG_KEY_IR_RIGHT == last_key
				   || GUI_MSG_KEY_LEFT == last_key)  //cai modify for ir  //DOWN  //cai modify for LEFT
				{
					...
					/*	|| GUI_MSG_KEY_VDEC == last_key*/   //絍VDEC替换为UP
					|| GUI_MSG_KEY_IR_LEFT == last_key)  //cai modify for ir   //UP
				{
					scene_para->cur_time_index--;
				...
				else if (/*GUI_MSG_KEY_LEFT == last_key ||*/ GUI_MSG_KEY_IR_DOWN == last_key)  //cai modify for ir  //LEFT  //cai modify for LEFT to DOWN
				{
				...
				else if (GUI_MSG_KEY_RIGHT == last_key || GUI_MSG_KEY_IR_UP == last_key) //cai modify for ir   //RIGHT
				{
					if (scene_para->cur_time_index < 0 || scene_para->cur_time_index >= 3)
					{
						__err("invalid para...\n");
			...
			else
			{
				switch(msg->dwAddData1)
				{
					...
					case GUI_MSG_KEY_LONGIR_DOWN:  //LEFT
					{
						if (scene_para->cur_time_index < 0 || scene_para->cur_time_index >= 3)
						{
							__err("invalid para...\n");
					...
					case GUI_MSG_KEY_LONGIR_UP: //RIGHT
					{
						if (scene_para->cur_time_index < 0 || scene_para->cur_time_index >= 3)
						{
							__err("invalid para...\n");
