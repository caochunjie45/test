...
static __s32 btn_info_str[] = {
	...
	STRING_MOVIE_PLAY,
	STRING_FM_TRACK,
	STRING_MOVIE_TRACK_1,
	STRING_MOVIE_JUMP,
	STRING_MOVIE_PLAYMODE,
	STRING_MOVIE_BRIGHT,
	STRING_MOVIE_SCREENRATE,
...
static __s32 show_size_str[] = 
{
	STRING_MOVIE_VIDEO_SIZE_FULL,
	STRING_MOVIE_SCR_SIZE_FULL,
	STRING_MOVIE_ORG_SIZE,
	STRING_MOVIE_4R3_SIZE,
	STRING_MOVIE_16R9_SIZE,
};

static __s32 track_num_str[] = 
{
	STRING_MOVIE_TRACK_1,
	STRING_MOVIE_TRACK_2,
	STRING_MOVIE_TRACK_3,
	STRING_MOVIE_TRACK_4,
};

static __u32 bright;
static __u32 scr_zoom;
static __u32 b_track;
...
static __s32 __movie_spsc_ctrl_static_icon_draw(movie_spsc_ctrl_scene_t *scene_para, movie_spsc_ctrl_static_icon_t type)
{
	...
	switch(type)    
	{
		...
		case movie_spsc_ctrl_static_icon_bright_text:
		{
			...
			bright = dsk_display_get_lcd_bright()+1;
		...
		case movie_spsc_ctrl_static_icon_btn_info:
		{
			char str[32]={0};
			GUI_RECT gui_rect;
        	movie_spsc_ctrl_uipara_t* ui_para;	
			...
			if(ui_para->focus_icon_index <= sizeof(btn_info_str))
			{
				if(ui_para->focus_icon_index == spsc_ctrl_scene_msg_channelset)
				{
					__audio_dev_chan_mode_t channelmode;
					
					channelmode = robin_get_channel();
	        		dsk_langres_get_menu_text(channel_mode_str[(__s32)channelmode], str, sizeof(str));
						
				}
				else if (ui_para->focus_icon_index == spsc_ctrl_scene_msg_playmode)
				{
					rat_play_mode_e playmode;
					
					playmode = robin_get_play_mode();
	        		dsk_langres_get_menu_text(play_mode_str[(__s32)playmode], str, sizeof(str));
				}
				else if (ui_para->focus_icon_index == spsc_ctrl_scene_msg_brightset)
				{
	        		dsk_langres_get_menu_text(bright_level_str[(__s32)bright], str, sizeof(str));
				}
				else if (ui_para->focus_icon_index == spsc_ctrl_scene_msg_screenset)
				{
	        		dsk_langres_get_menu_text(show_size_str[(__s32)scr_zoom], str, sizeof(str));
				}
				else if (ui_para->focus_icon_index == spsc_ctrl_scene_msg_trackset)
				{
	        		dsk_langres_get_menu_text(track_num_str[(__s32)b_track], str, sizeof(str));
				}				
				else
	        		dsk_langres_get_menu_text(btn_info_str[ui_para->focus_icon_index], str, sizeof(str));
				...
				gui_rect.x0 = ui_para->uipara_static_icon[movie_spsc_ctrl_static_icon_btn_info].x;
				gui_rect.y0 = ui_para->uipara_static_icon[movie_spsc_ctrl_static_icon_btn_info].y;
				gui_rect.x1 = gui_rect.x0+ui_para->uipara_static_icon[movie_spsc_ctrl_static_icon_btn_info].w;
				gui_rect.y1 = gui_rect.y0+ui_para->uipara_static_icon[movie_spsc_ctrl_static_icon_btn_info].h;
				GUI_ClearRectEx(&gui_rect);
...
static __s32 __movie_spsc_ctrl_notify_parent(movie_spsc_ctrl_scene_t* scene_para, __gui_msg_t* msg, movie_spsc_ctrl_icon_t icon_id)
{
	switch(icon_id)
	{
		...
		case movie_spsc_ctrl_icon_rr:
		{
			...
			{
				movie_cmd2parent(msg->h_deswin, spsc_ctrl_scene_msg_playpause, 1, 1);
			}
			else
			{
		...
		case movie_spsc_ctrl_icon_channelset:
		{
			...
			__movie_spsc_ctrl_static_icon_draw(scene_para, movie_spsc_ctrl_static_icon_btn_info);
		...
		{			
            ...
            b_track = track = (robin_get_track_index()+1)%track_para.nAudStrmNum;            
		...
		case movie_spsc_ctrl_icon_brightset:
		{
			...
			if(bright>14)
				bright = 0;
			movie_cmd2parent_syn(msg->h_deswin, spsc_ctrl_scene_msg_brightset, bright, 0);//后面需要立即更新ui，需发送同步消息

			GUI_LyrWinSel(scene_para->hlyr);	
			__movie_spsc_ctrl_static_icon_draw(scene_para, movie_spsc_ctrl_static_icon_bright);
			__movie_spsc_ctrl_static_icon_draw(scene_para, movie_spsc_ctrl_static_icon_bright_text);
			__movie_spsc_ctrl_static_icon_draw(scene_para, movie_spsc_ctrl_static_icon_btn_info);
		...
		case movie_spsc_ctrl_icon_screenset:
		{
			...
			if(scr_zoom>4)
				scr_zoom = 0;
			movie_cmd2parent_syn(msg->h_deswin, spsc_ctrl_scene_msg_screenset, scr_zoom, 0);//后面需要立即更新ui，需发送同步消息
			__movie_spsc_ctrl_static_icon_draw(scene_para, movie_spsc_ctrl_static_icon_btn_info);
			GUI_LyrWinSel(scene_para->hlyr);	
...
static __s32 __movie_spsc_ctrl_proc(__gui_msg_t* msg)
{
	switch(msg->id)
	{
		case GUI_MSG_CREATE:
		{
			...
			scr_zoom = robin_get_zoom();
			b_track = robin_get_track_index();	
		...
		case GUI_MSG_KEY:
		{
			...
			{
				if (GUI_MSG_KEY_LEFT == last_key || GUI_MSG_KEY_IR_LEFT == last_key)  //cai modify for ir   //LEFT
				{
				...
				else if (GUI_MSG_KEY_RIGHT == last_key || GUI_MSG_KEY_IR_RIGHT == last_key)  //cai modify for ir  //RIGHT
				{
					__msg("draw unfocus icon..\n");
                    GUI_LyrWinSel(scene_para->hlyr);
					com_memdev_create(scene_para->hlyr);
			...
			else
			{
				switch(msg->dwAddData1)
				{
					...
					case GUI_MSG_KEY_LONGIR_LEFT:   //cai++  //LONGLEFT
					{
						__msg("draw unfocus icon..\n");
                        GUI_LyrWinSel(scene_para->hlyr);
						com_memdev_create(scene_para->hlyr);
					...
					case GUI_MSG_KEY_LONGIR_RIGHT:    //cai++   //LONGLEFT
					{
						__msg("draw unfocus icon..\n");
                        GUI_LyrWinSel(scene_para->hlyr);
						com_memdev_create(scene_para->hlyr);
