...
static __s32 __bright_proc(__gui_msg_t* msg)
{
	switch(msg->id)
	{
		...
		case GUI_MSG_KEY:
		{
			...
			{
				if (GUI_MSG_KEY_LEFT == last_key || GUI_MSG_KEY_IR_LEFT == last_key)  //cai modify for ir 
				{
				...
				else if (GUI_MSG_KEY_RIGHT == last_key || GUI_MSG_KEY_IR_RIGHT == last_key)  //cai modify for ir    
				{
					__bright_reset_hide_timmer(scene_para);
			...
			else
			{
				switch(msg->dwAddData1)
				{
					...
					case GUI_MSG_KEY_LONGIR_LEFT:   //cai++ for ir
					{
						__bright_reset_hide_timmer(scene_para);
						if (scene_para->cur_val > scene_para->min_val)
						{
					...
					case GUI_MSG_KEY_LONGIR_RIGHT:   //cai++ for ir
					{
						__bright_reset_hide_timmer(scene_para);
						if (scene_para->cur_val < scene_para->max_val)
						{
