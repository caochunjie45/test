...
typedef struct tag_setting_power_attr
{
	...
	//cai++ for setting power modify or not
	__bool	power_setting_init_flag;
	__s32	original_content_num[3];
	__s32	new_content_num[3];
...
static __s32 setting_power_item_paint(__gui_msg_t *msg)
{
	...
	for(i=0; i<power_attr->item_nr; i++)
	{
		switch (i)
		{
			case 0:
				...
				if(power_attr->power_setting_init_flag)   //cai++ for setting modify or not
					power_attr->original_content_num[0] = p_item_res->content_num;
				else
					power_attr->new_content_num[0] = p_item_res->content_num;				
				break;
			case 1:
				p_item_res = &power_attr->res_backlight_off;

				if(power_attr->power_setting_init_flag)   //cai++ for setting modify or not
					power_attr->original_content_num[1] = p_item_res->content_num;
				else
					power_attr->new_content_num[1] = p_item_res->content_num;	
				break;
			case 2:
				p_item_res = &power_attr->res_power_off;

				if(power_attr->power_setting_init_flag)   //cai++ for setting modify or not
					power_attr->original_content_num[2] = p_item_res->content_num;
				else
					power_attr->new_content_num[2] = p_item_res->content_num;	
				break;		
			default:
				break;
		}
	...
	power_attr->power_setting_init_flag = 0; //cai++ for power_setting_modify
	return EPDK_OK;
}
	...
	case GUI_MSG_CREATE:
		{
			...
			power_attr->power_setting_init_flag = 1; //cai++ for power_setting_modify_or_not
	...
	case GUI_MSG_DESTROY:
		{
			...
			//dsk_reg_flush();
			//判断是否修改过//cai++
			/*if(power_attr->new_content_num[0] != power_attr->original_content_num[0]
			 || power_attr->new_content_num[1] != power_attr->original_content_num[1]
			 || power_attr->new_content_num[2] != power_attr->original_content_num[2])
			{
	...
	case MSG_OP_ENTER:
		{
			...
			power_attr->new_content_num[power_attr->new_focus] = p_item_res->content_num;
			setting_power_content_paint( msg);
			switch (power_attr->new_focus)
			{
				case 0:
