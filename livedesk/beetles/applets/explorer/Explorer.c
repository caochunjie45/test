...
{
	H_WIN h_explr;
	//H_LYR BG_layer;			//background layer 8bpp(注意背景图需要设置为8bpp)  //cai modify for 背景全屏
...
static __s32 explorer_scene_create(__gui_msg_t *msg)
{
    ...
    else
    {
        ...
        __msg("\n\n~~~ explr_ctrl->root=%s ~~~\n\n", explr_ctrl->root);	
//	explr_ctrl->BG_layer= explorer_8bpp_layer_create();  //cai modify for 背景全屏
	explr_ctrl->list_layer = explorer_32bpp_layer_create();

//    explorer_draw_bg(explr_ctrl);   //cai modify for 背景全屏

//	ListPara.BG_layer = explr_ctrl->BG_layer;		//背景图层  //cai modify
    ...
    //GUI_LyrWinSetSta(explr_ctrl->BG_layer, GUI_LYRWIN_STA_ON);  //cai modify
      // gscene_bgd_set_state(BGD_STATUS_SHOW);   //cai++ for 背景图层
