...
{    
   ...
   //     { 13, 3 + 71 - 35, 100, 30},      	//title rect
    ...
    {				//ID															res	handle,    	res buffer
        ...
        { ID_EXPLORER_FOLDER_BMP,      	NULL,  			 NULL }, 	//  video文件标志                                                     				
        ...
        { ID_EXPLORER_MUSIC_FILE_BMP,      		NULL,  				 NULL }, 	// karaoke文件标志                                                     				
		{ ID_EXPLORER_MUSIC_ICON_UNFOCUSED_BMP, NULL,  				 NULL }, 	// karaoke文件icon图标,与文件名一起的                                                      				        
		{ ID_EXPLORER_MUSIC_ICON_FOCUSED_BMP, 	NULL,  				 NULL }, 	// karaoke文件icon图标,与文件名一起的                                                      				        		
#endif
...
__s32 explorer_draw_FileTypeIcon(explr_list_para_t *list_para, rat_media_type_t media_type)
{
	...
	case RAT_MEDIA_TYPE_KARAOKE:	
		__wrn("explorer_draw_FileTypeIcon \n");
		pic_buf = explorer_get_listview_icon_res(ID_EXP_KARAOKE_FILE);			//draw select border
		break;	
...
void *explorer_get_file_item_icon(explorer_view_mode_e view_mode, rat_media_type_t media_type, exp_icon_status_e status)
{
	...
	switch(media_type)
	{
		...
		case RAT_MEDIA_TYPE_KARAOKE:	
			if(status == FOCUSED)
			{
				pic_buf = explorer_get_listview_icon_res(ID_EXP_MUSIC_ICON_FOCUSED);//ID_EXP_KARAOKE_ICON_FOCUSED	 // 112350		
			}
			else
			{
				pic_buf = explorer_get_listview_icon_res(ID_EXP_MUSIC_ICON_UNFOCUSED);//ID_EXP_KARAOKE_ICON_UFFOCUSED		 // 112350		
			}
			break;				
#endif			
		default:
