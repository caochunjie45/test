...
static __s32 content_lcn_mode_id[][2] =
{
	 {STRING_DTV_SET_LCN_OFF, LCN_OFF},
	 {STRING_DTV_SET_LCN_ON, LCN_ON},
};
...
static void dtv_setting_update_top_view(dtv_setting_attr_t *gattr)
{
	...
	dsk_langres_get_menu_text(STRING_DTV_SET_LCN_EX,tmp,sizeof(tmp));
...
static __s32 dtv_setting_item_paint_ex(__lbar_draw_para_t *draw_param)
{
	...
	switch(draw_param->mode)
	{
		case LBAR_MODE_NORMAL:
		{
			...
			dsk_langres_get_menu_text(content_lcn_mode_id[draw_param->index][0],shdl_name,sizeof(shdl_name));
	   ...
	   case GUI_MSG_KEY_LONGLEFT:		   	
	   {
	       if((msg->dwAddData2 == KEY_DOWN_ACTION) || (msg->dwAddData2 == KEY_REPEAT_ACTION))
	       {
				...
				if(attr->new_focus >=0 && attr->new_focus <= attr->total)
					 maple_set_lcn_state(content_lcn_mode_id[attr->new_focus][1]);
	   	   }
	       break;
	   }
...
static __s32 _dtv_setting_Proc(__gui_msg_t *msg)
{
	switch(msg->id)
	{
		case GUI_MSG_CREATE:
		{
			...
			para = (dtv_setting_lcn_para_t *)GUI_WinGetAttr(msg->h_deswin);
			...
			attr->total = sizeof(content_lcn_mode_id)/sizeof(content_lcn_mode_id[0]);
		...
		case GUI_MSG_DESTROY:
		{
			...
			if(para)
			{
				My_Bfree(para, sizeof(dtv_setting_lcn_para_t));
				para = NULL;
			}
			if(attr)
			{
		...
		case GUI_MSG_KEY:		
			return _dtv_setting_listbar_key_proc(msg);
		
		case GUI_MSG_TOUCH:		
			return dtv_setting_touch_proc(msg);
...
H_WIN dtv_setting_lcn_win_create(H_WIN h_parent, dtv_setting_lcn_para_t *list_para)
{
	...
	para = (dtv_setting_lcn_para_t *)My_Balloc(sizeof(dtv_setting_lcn_para_t));
	if(!para)
	{
		__msg("memory balloc fail.........\n");
		return EPDK_FAIL;
	}
	eLIBs_memset((void*)para, 0, sizeof(dtv_setting_lcn_para_t));
	para->font = SWFFont;
	para->layer = list_para->layer;
	para->focus_id = list_para->focus_id;
	eLIBs_memset(&framewin_para, 0, sizeof(__gui_framewincreate_para_t));
	framewin_para.name =	SUBMENU_DVB_SETTING_LCN;
	framewin_para.dwExStyle = WS_EX_NONE;
	framewin_para.dwStyle = WS_NONE|WS_VISIBLE;
	framewin_para.spCaption =  NULL;
	framewin_para.hOwner	= NULL;
	framewin_para.id         = SUBMENU_DVB_SETTING_LCNID;
...
__s32 dtv_setting_lcn_win_delete(H_WIN list_win)
{
