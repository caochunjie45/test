...
typedef struct tag_ebook_mset_ctrl
{
	...
	ebook_menu_mode_t menu_mode;  //cai++
}ebook_mset_ctrl_t;
...
static __s32 ebook_mset_key_proc(__gui_msg_t *msg)
{
	...
	if(KEY_UP_ACTION==msg->dwAddData2)
	{
		if ((last_key == GUI_MSG_KEY_ENTER) || (last_key ==  GUI_MSG_KEY_LONGENTER))
		{
			...
			if(mset_ctrl->menu_mode == EBOOK_MAIN_MENU)
			{				
				Ebook_cmd2parent(msg->h_deswin, CMD_EBOOK_MSET_SW_ITEM, mset_ctrl->focus_id, 0);
				mset_ctrl->menu_mode = EBOOK_SUB_MENU;
			}
			else if(mset_ctrl->menu_mode == EBOOK_SUB_MENU)
			{
				Ebook_cmd2parent(msg->h_deswin, CMD_EBOOK_MSET_ENTER, 0, 0);
			}
		}
		else if (last_key == GUI_MSG_KEY_ESCAPE || last_key == GUI_MSG_KEY_MENU)//退出菜单显示场景
		{
			if(mset_ctrl->menu_mode == EBOOK_MAIN_MENU)
			{
			...
			{
				GUI_ResetTimer(msg->h_deswin, Timer_msetStaId, 700, 0);
				Ebook_cmd2parent(msg->h_deswin, CMD_EBOOK_MSET_SW_ITEM, ID_EBOOK_MSET_UNKOWN, 0);
				mset_ctrl->menu_mode = EBOOK_MAIN_MENU;
	...
	else
	{
		switch(msg->dwAddData1)
		{
			...
			case GUI_MSG_KEY_LONGLEFT:
			{
				if(mset_ctrl->menu_mode == EBOOK_MAIN_MENU)
				{
					...
					else
					{
						...
						//Ebook_cmd2parent(msg->h_deswin, CMD_EBOOK_MSET_SW_ITEM, mset_ctrl->focus_id, 0);  //cai modify
					}
				}
				else if(mset_ctrl->menu_mode == EBOOK_SUB_MENU)
				{
					if (mset_ctrl->lr_adjust_sset == 1)   //页面跳转时使用
					{
						GUI_ResetTimer(msg->h_deswin, Timer_msetStaId, 700, 0);
			...
			case GUI_MSG_KEY_DOWN:	//GUI_MSG_KEY_VADD//短按上下切换  //将VADD替换为DOWN
			{
				if (mset_ctrl->lr_adjust_sset == 1)
				{
					GUI_ResetTimer(msg->h_deswin, Timer_msetStaId, 700, 0);
			...
			case GUI_MSG_KEY_LONGIR_LEFT:
			{
				...
				else if(mset_ctrl->menu_mode == EBOOK_SUB_MENU)
				{
					...
					{
						GUI_ResetTimer(msg->h_deswin, Timer_msetStaId, 700, 0);
						Ebook_cmd2parent(msg->h_deswin, CMD_EBOOK_MSET_LEFT, 0, 0);
					}
				}
			}
				break;
