...
static __s32 _rec_view_frm_on_key(__gui_msg_t *msg)
{
	...
	if (KEY_UP_ACTION != msg->dwAddData2)
	{
		...
		if(pviewFrm_ctl->pview->menu_mode == RECORD_MAIN_MENU)
		{
				...
				pviewFrm_ctl->pview->menu_mode = RECORD_SUB_MENU;
				pviewFrm_ctl->pview->cursubmenu = SUBMENU_QUALITY;
				...
				pviewFrm_ctl->pview->curMenuSt = FOCUSE_PRESS;
				pviewFrm_ctl->pview->menu_mode = RECORD_SUB_MENU;
				pviewFrm_ctl->pview->cursubmenu = SUBMENU_FORMAT;
	        	#if 0
	        		//pviewFrm_ctl->pview->curMenuSt = FOCUSE_OVER;
	        		//pviewFrm_ctl->pview->curMenu   = MENU_RECORD;
		...
		else if(pviewFrm_ctl->pview->menu_mode == RECORD_SUB_MENU)  //cai++
		{
			if(pviewFrm_ctl->pview->cursubmenu == SUBMENU_QUALITY)
			{
	        		//pviewFrm_ctl->pview->curMenuSt = FOCUSE_OVER;
	        		//pviewFrm_ctl->pview->curMenu   = MENU_RECORD;
	        		pviewFrm_ctl->precordDoc->recQuality = pviewFrm_ctl->precordDoc->recQualityTemp;
			...
			else if(pviewFrm_ctl->pview->cursubmenu == SUBMENU_FORMAT)
			{
	        		//pviewFrm_ctl->pview->curMenuSt = FOCUSE_OVER;
	        		//pviewFrm_ctl->pview->curMenu   = MENU_RECORD;
	        		pviewFrm_ctl->precordDoc->audioType = pviewFrm_ctl->precordDoc->audioTypeTemp;
			...
			break;
		case GUI_MSG_KEY_DOWN:     //将VADD功能替换为DOWN
		...
		case GUI_MSG_KEY_LEFT:
			...
			else if(pviewFrm_ctl->pview->menu_mode == RECORD_SUB_MENU)
			{
		        	if( pviewFrm_ctl->pview->curMenu == MENU_QUALITY &&
		        		pviewFrm_ctl->pview->curMenuSt == FOCUSE_PRESS && pviewFrm_ctl->pview->cursubmenu == SUBMENU_QUALITY)
		        	{
		        		pviewFrm_ctl->precordDoc->recQualityTemp = (QUALITY_STATE)((pviewFrm_ctl->precordDoc->recQualityTemp+1)%QUALITY_STATE_MAX);
		        	}
		        	else if( pviewFrm_ctl->pview->curMenu == MENU_FORMAT &&
		            		pviewFrm_ctl->pview->curMenuSt == FOCUSE_PRESS && pviewFrm_ctl->pview->cursubmenu == SUBMENU_FORMAT)
		        	{
		        		pviewFrm_ctl->precordDoc->audioTypeTemp = (RECORD_AUDIO_TYPE)((pviewFrm_ctl->precordDoc->audioTypeTemp+1)%RECORD_AUDIO_TYPE_MAX);
		        	}
			}
		...
		case GUI_MSG_KEY_RIGHT:
			...
			else if(pviewFrm_ctl->pview->menu_mode == RECORD_SUB_MENU)
			{
		        		...
		        		pviewFrm_ctl->pview->curMenuSt == FOCUSE_PRESS && pviewFrm_ctl->pview->cursubmenu == SUBMENU_QUALITY)
		        	{
						...
						pviewFrm_ctl->precordDoc->recQualityTemp = QUALITY_STATE_MAX -1;
					else	
		        			pviewFrm_ctl->precordDoc->recQualityTemp--;
		        	}
		        	else if( pviewFrm_ctl->pview->curMenu == MENU_FORMAT &&
		            		pviewFrm_ctl->pview->curMenuSt == FOCUSE_PRESS && pviewFrm_ctl->pview->cursubmenu == SUBMENU_FORMAT)
		        	{
		        		if(pviewFrm_ctl->precordDoc->audioTypeTemp == 0)
		        			pviewFrm_ctl->precordDoc->audioTypeTemp = RECORD_AUDIO_TYPE_MAX -1;
					else
						pviewFrm_ctl->precordDoc->audioTypeTemp --;
				}				
			}
			break;
		...
		{
			if(pviewFrm_ctl->pview->menu_mode == RECORD_SUB_MENU)
			{
		        	if( pviewFrm_ctl->pview->curMenu == MENU_QUALITY &&
		        		pviewFrm_ctl->pview->curMenuSt == FOCUSE_PRESS && pviewFrm_ctl->pview->cursubmenu == SUBMENU_QUALITY)
		        	{
	...
	else
	{
		switch( msg->dwAddData1 )
		{
			case GUI_MSG_KEY_ESCAPE://sun++
			if(pviewFrm_ctl->pview->cursubmenu == SUBMENU_QUALITY || pviewFrm_ctl->pview->cursubmenu == SUBMENU_FORMAT)
			{
				pviewFrm_ctl->pview->menu_mode = RECORD_MAIN_MENU;
				pviewFrm_ctl->pview->cursubmenu = SUBMENU_UNKNOW;
			}
			else
...
static __s32 _rec_view_frm_manager_proc(__gui_msg_t *msg)
{
    ...
    case RECORD_VIEW_FRAME_STARTRECORD:		//开始录音
    {
		...
		ioecho = get_gpio_status(ECHO_PORT, ECHO_PORTNUM);		
