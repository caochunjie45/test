...
static void record_set_path(precord_doc_t precord_doc)
{
	...
	{
		SLIB_strcat(precord_doc->recordName, "20140101");
	}
	else
	{
...
__s32 record_start(precord_doc_t precord_doc)
{
	...
	ret = esMODS_MIoctrl(precord_doc->mod_ginkgo, GINKGO_CMD_SET_AUDIOGAIN, 10, 0);
	if(ret < 0)
	{
		__wrn("----set audiogain fail\n");
	}
	else
	{
		__msg("----set audiogain ok!\n");
	}
#endif
...
#if BEETLES_IOECHO_CHECK
	ret = esMODS_MIoctrl(precord_doc->mod_ginkgo, GINKGO_CMD_SET_AUDIOGAIN, 0, 0);
	if(ret < 0)
	{
		__wrn("----set audiogain fail\n");
	}
