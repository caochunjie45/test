...
static void record_view_Top( precord_view_t prec_view )
{
	...
	dsk_power_get_voltage_level(&batterySt,&is_full);		
#else //带PMU
	dsk_power_get_battery_level(&batterySt,&is_full);
#endif

	if ((charge_state == 1) && !is_full)	//充电状态	
	{
		if(++Draw_cnt > 2)
		{
			Bat_cnt ++;
		...
		draw_record_bmp( prec_view->pui, RECORD_BATTERY_ADD0_BMP+Bat_cnt);
...
static void record_view_recInfo( precord_view_t prec_view )
{
	...
	draw_record_bmp_ext( prec_view->pui, curRecTimes[0]-'0'+RECORD_0_BMP, offsetX, offsetY );offsetX+=22;
	draw_record_bmp_ext( prec_view->pui, curRecTimes[1]-'0'+RECORD_0_BMP, offsetX, offsetY );offsetX+=22;

	draw_record_bmp_ext( prec_view->pui, curRecTimes[2]-'0'+RECORD_0_BMP, offsetX, offsetY );offsetX+=22;

	draw_record_bmp_ext( prec_view->pui, curRecTimes[3]-'0'+RECORD_0_BMP, offsetX, offsetY );offsetX+=22;
	draw_record_bmp_ext( prec_view->pui, curRecTimes[4]-'0'+RECORD_0_BMP, offsetX, offsetY );offsetX+=22;

	draw_record_bmp_ext( prec_view->pui, curRecTimes[5]-'0'+RECORD_0_BMP, offsetX, offsetY );offsetX+=22;

	draw_record_bmp_ext( prec_view->pui, curRecTimes[6]-'0'+RECORD_0_BMP, offsetX, offsetY );offsetX+=22;
	draw_record_bmp_ext( prec_view->pui, curRecTimes[7]-'0'+RECORD_0_BMP, offsetX, offsetY );offsetX+=22;
	//录制文件信息
...
static void record_view_menuBox( precord_view_t prec_view )
{
	if( prec_view->curMenuSt == FOCUSE_PRESS )
	{
		...
		{
		case MENU_QUALITY:
			if(prec_view->cursubmenu == SUBMENU_QUALITY)
			{
		...
		case MENU_MUSIC_LIST:
			break;
		case MENU_FORMAT:
			if(prec_view->cursubmenu == SUBMENU_FORMAT)
			{
