...
static __bool photo_exit_flag = 0;
/*********************************************************************************************************************************************************
*
*		向父窗口发信息			
*
...
static __s32 sset_win_create(__gui_msg_t *msg, __s32 mset_id)
{
	...
	switch(mset_id)
	{
		...
		case ID_PHOTO_MSET_BRIGHT:
			...
			sset_para.value = get_sys_bright()-1;//(get_sys_bright())*11 /15;
			break;
...
static __s32 mset_win_create(__gui_msg_t *msg)
{
	...
	mset_para.bright = get_sys_bright()-1;
...
static __s32 photo_spsc_cmd_proc(__gui_msg_t  *msg)
{
	...
	switch(HIWORD(msg->dwAddData1))
	{
		...
		case CMD_PHOTO_SPSC_SET:
		{
			...
			if(photo_ctrl->menu_focus_id == ID_PHOTO_MSET_SAVE_BG || photo_ctrl->menu_focus_id == ID_PHOTO_MSET_ROTATE)
			{
				__msg("focus_id = %d\n", photo_ctrl->menu_focus_id);
				sset_win_create(msg, photo_ctrl->menu_focus_id);  //cai modify
			}
			break;
		}
...
static __s32 photo_mset_cmd_proc(__gui_msg_t  *msg)
{
	...
	switch(HIWORD(msg->dwAddData1))
	{
		...
		case CMD_PHOTO_MSET_ENTER:
		{
			...
			setmsg.h_deswin = photo_ctrl->h_photo_sset;
			setmsg.dwAddData1= msg->dwAddData2;   //cai++
			GUI_SendMessage(&setmsg);
			break;
		}
...
static __s32 photo_sset_cmd_proc(__gui_msg_t  *msg)
{
	...
	switch(HIWORD(msg->dwAddData1))
	{
		case CMD_PHOTO_SSET_SOURCE:
		{
			...
			__s32 str[] = {STRING_MAINMENU_PHOTO, STRING_CONFIRM_BG_SET};
			__here__;
...
// 			}
		...
		case CMD_PHOTO_SSET_BRIGHT:
		{
			...
			bright = msg->dwAddData2;//(msg->dwAddData2 *15) / 11;
...
{DEBUG_CBMSGEx(_app_photo_Proc);
	switch(msg->id)
    {
				...
				case ADLG_CMD_EXIT:
					{
						...
						{
							__s32 str[] = {STRING_MAINMENU_PHOTO, STRING_PHOTO_BG_SAVED};
							photo_save_bg();
		...
		case GUI_MSG_KEY:	
		{
			...
			{
				if (msg->dwAddData1 == GUI_MSG_KEY_MENU || msg->dwAddData1 == GUI_MSG_KEY_LONGMENU || msg->dwAddData1 == GUI_MSG_KEY_IR_MENU || msg->dwAddData1 == GUI_MSG_KEY_LONGIR_MENU)
				{
