			...
			if (msg->dwAddData1 != GUI_MSG_KEY_LONGLEFT && msg->dwAddData1 != GUI_MSG_KEY_LONGRIGHT)
			{
				if (ANOLE_GetSta() == ANOLE_MODE_ZOOM)
				{
					if (msg->dwAddData1 == GUI_MSG_KEY_LONGMENU || msg->dwAddData1 == VK_POP_VOLUME
						|| msg->dwAddData1 == GUI_MSG_KEY_IR_MENU || msg->dwAddData1 == GUI_MSG_KEY_LONGIR_MENU)
					{//向下传做处理(放大时上下长按)
					}
					else
					{
...
__here__;
	...
	{//增加放大时的处理
		...
		{
			if (/*msg->dwAddData1 == GUI_MSG_KEY_ESCAPE      //退出用作向下   //cai++ 去掉此处  使得能够正常退出菜单
				||*/ msg->dwAddData1 == GUI_MSG_KEY_LONGMENU
				|| msg->dwAddData1 == GUI_MSG_KEY_IR_MENU
				|| msg->dwAddData1 == GUI_MSG_KEY_LONGIR_MENU)
			{
				if (ANOLE_GetSta() == ANOLE_MODE_ZOOM)
				{
	...
	switch(msg->dwAddData1)
	{
		...
		case GUI_MSG_KEY_LONGLEFT:
			{
				if(mset_ctrl->menu_mode == PHOTO_MAIN_MENU)
				{
					...
					{
					__here__;
						if(mset_ctrl->focus_id == ID_PHOTO_MSET_ROTATE)
						{
							photo_cmd2parent(msg->h_deswin, CMD_PHOTO_MSET_SW_ITEM, ID_PHOTO_MSET_UNKNOW, 0);  //cai modify
						}
						...
						if(mset_ctrl->focus_id == ID_PHOTO_MSET_ROTATE)
						{
							photo_cmd2parent(msg->h_deswin, CMD_PHOTO_MSET_SW_ITEM, mset_ctrl->focus_id, 0);  //cai modify
						}
					}					
				}
				else if(mset_ctrl->menu_mode == PHOTO_SUB_MENU)
				{
					__here__;
					if(mset_ctrl->lr_adjust_sset == 1 && mset_ctrl->focus_id != ID_PHOTO_MSET_INTERVAL)
					{
						GUI_ResetTimer(msg->h_deswin, Timer_msetStaId, DELAY_MSETSTAID, 0);
		...
		case GUI_MSG_KEY_LONGRIGHT:
			{
				if(mset_ctrl->menu_mode == PHOTO_MAIN_MENU)
				{
					...
					__here__;
						...
						GUI_ResetTimer(msg->h_deswin, Timer_msetStaId, DELAY_MSETSTAID, 0);
						photo_mset_paint(msg, PAINT_PHOTO_MSET_ALL);//photo_mset_paint(msg, PAINT_PHOTO_MSET_ITEM);
						if(mset_ctrl->focus_id == ID_PHOTO_MSET_SAVE_BG || mset_ctrl->focus_id == ID_PHOTO_MSET_ROTATE)
						{
							photo_cmd2parent(msg->h_deswin, CMD_PHOTO_MSET_SW_ITEM, mset_ctrl->focus_id, 0);  //cai modify
						}
					}
		...
		case GUI_MSG_KEY_LONGENTER:
			{
				...
				if(mset_ctrl->menu_mode == PHOTO_MAIN_MENU)
				{
					...
					{
						__here__;
						photo_cmd2parent(msg->h_deswin, CMD_PHOTO_MSET_ENTER, mset_ctrl->focus_id, 0);
					}
					else
					{
						photo_cmd2parent(msg->h_deswin, CMD_PHOTO_MSET_SW_ITEM, mset_ctrl->focus_id, 0);
						mset_ctrl->menu_mode = PHOTO_SUB_MENU;
					}
				}
				else if(mset_ctrl->menu_mode == PHOTO_SUB_MENU)
				{
		...
		case GUI_MSG_KEY_MENU:
			{
				...
				else if(mset_ctrl->menu_mode == PHOTO_SUB_MENU)
				{
					...
					photo_cmd2parent(msg->h_deswin, CMD_PHOTO_MSET_SW_ITEM, ID_PHOTO_MSET_UNKNOW, 0);  //cai modify
					mset_ctrl->lr_adjust_sset = 0;  //cai++
					mset_ctrl->menu_mode = PHOTO_MAIN_MENU;
				}
		...
		case GUI_MSG_KEY_LONGIR_LEFT:
			{	
				...
				else if(mset_ctrl->menu_mode == PHOTO_SUB_MENU)
				{
					...
					else if(mset_ctrl->lr_adjust_sset == 1 && mset_ctrl->focus_id == ID_PHOTO_MSET_INTERVAL)
					{
						__here__;
						GUI_ResetTimer(msg->h_deswin, Timer_msetStaId, DELAY_MSETSTAID, 0);
						photo_cmd2parent(msg->h_deswin, CMD_PHOTO_MSET_DOWN, 0, 0);						
		...
		case GUI_MSG_KEY_LONGIR_UP:
			...
			__here__;
				if(mset_ctrl->lr_adjust_sset == 1 && mset_ctrl->focus_id == ID_PHOTO_MSET_INTERVAL)
				{
					if(mset_ctrl->menu_mode == PHOTO_SUB_MENU)
					{
						GUI_ResetTimer(msg->h_deswin, Timer_msetStaId, DELAY_MSETSTAID, 0);
						photo_cmd2parent(msg->h_deswin, CMD_PHOTO_MSET_RIGHT, 0, 0);					
					}
