...
//显示左上角文字
//当前频道:
	GUI_BMP_RES_Draw(res->bmp_ch, 35, 38);//显示"ch"
	...
	GUI_BMP_RES_Draw(res->bmp_l[b], 70, 38);//显示十位
	GUI_BMP_RES_Draw(res->bmp_l[c], 82, 38);//显示个位
	
//频道总数:	
	
	GUI_BMP_RES_Draw(res->bmp_slash, 90, 38);  //显示斜杠"/"
	b = fm_ctrl->channel_count /10;
	c = fm_ctrl->channel_count % 10;
	GUI_BMP_RES_Draw(res->bmp_l[b], 102, 38);//显示十位
	GUI_BMP_RES_Draw(res->bmp_l[c], 114, 38);//显示个位

//显示右上角文字
//当前频道:
	GUI_BMP_RES_Draw(res->bmp_preset, 325, 36);//显示"preset"
	if ( fm_ctrl->channel_count > 0)
		{
			b =(fm_ctrl->channel_id +1) /10;
			c =(fm_ctrl->channel_id +1) % 10;	
	...
	GUI_BMP_RES_Draw(res->bmp_m[b],405,38 );	//显示十位
	GUI_BMP_RES_Draw(res->bmp_m[c],420,38);	//显示个位
//显示中心文字
...
//当前频段:
	...
	if(a>0)
	{		
		GUI_BMP_RES_Draw(res->bmp_b[a], 165,98);	// 显示百位
	}
		
	GUI_BMP_RES_Draw(res->bmp_b[b], 185,98);		//显示十位
	GUI_BMP_RES_Draw(res->bmp_b[c], 205,98);		//显示个位
	GUI_BMP_RES_Draw(res->bmp_point, 225,118);		//显示小数点
	GUI_BMP_RES_Draw(res->bmp_b[d], 230,98);		//显示小数点后一位	
	GUI_BMP_RES_Draw(res->bmp_MHZ, 252,97);   		//显示"MHZ"
	
//卡尺下频段文字
#if  (FM_SEARCH_CHN_MIN_FREQ == 87500)	
	GUI_BMP_RES_Draw(res->bmp_87_5MHZ, 13,172);		//显示"87.5MHZ"	
#else if  (FM_SEARCH_CHN_MIN_FREQ == 87000)	
	GUI_BMP_RES_Draw(res->bmp_87MHZ, 13,172);		//显示"87MHZ"	
#endif
	GUI_BMP_RES_Draw(res->bmp_108MHZ, 393,172);	//显示"108MHZ"
...
//声音模式文字	
	...
	if(fm_ctrl->audio_mode == 1)
		{
			GUI_BMP_RES_Draw(res->bmp_mono,209,172);		//显示"mono"
		}
	else if(fm_ctrl->audio_mode == 2)
		{
			GUI_BMP_RES_Draw(res->bmp_stereo,209,172);		//显示"stereo"
		}
