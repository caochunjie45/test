...
{	
	...
	{
		pic_buf = music_get_icon_res(ID_MUSIC_VOL_MUTE_ICON);	
	}
...
__s32 DrawMusicVolumeMute(void)
{	
	void *pic_buf = NULL;
	HTHEME	res_handle;
	__s32 X = 0, Y = 0;
	...
	X = headbar_ui->volume_icon_rect.x;
	Y = headbar_ui->volume_icon_rect.y;
	width = X + headbar_ui->volume_icon_rect.width;
	height = Y + headbar_ui->volume_icon_rect.height;

        res_handle = theme_open(ID_MUSIC_VOLUME_MUTE_BMP);  //获取静音图片
        pic_buf  = theme_hdl2buf(res_handle);         
		
	if(pic_buf != NULL)
	{
		GUI_ClearRect(X,Y,width,height);
		GUI_BMP_Draw(pic_buf, X, Y);	
	}
...
__s32 DrawMusicBatteryStatus(power_level_e level, __s32 is_full)
{	
	void *pic_buf = NULL;
	__s32 X = 0, Y = 0;	
	music_headbar_ui_t *headbar_ui;
	...
	__msg(".......555..........charge_status=%d, is_full = %d ......\n",charge_status, is_full);
    if(EPDK_TRUE== charge_status && !is_full)
    {
	__msg(".......666..........Bat_cnt=%d,level = %d  ......\n",Bat_cnt, level);
...
//Headbar显示盘符
__s32 DrawMusicSdOrUsb(void)
{	
	music_headbar_ui_t *headbar_ui;
	GUI_RECT str_rect;
	char disk_text[12];
	__u32 cur_type, root_type;
	__u32 sd_root =0;
	__u32 usb_root = 0;
	...
	gscene_hbar_get_disk(&cur_type, &root_type);
	...
	//eLIBs_printf("### %s, %d, sd_root = %x, usb_root = %x ###\n", __FILE__, __LINE__, sd_root, usb_root);

	//sd
	str_rect.x0 = headbar_ui->sd_rect.x;
	str_rect.y0 = headbar_ui->sd_rect.y;
	str_rect.x1 = str_rect.x0 + headbar_ui->sd_rect.width;
	str_rect.y1 = str_rect.y0 + headbar_ui->sd_rect.height;

	GUI_ClearRect(str_rect.x0, str_rect.y0, str_rect.x1, str_rect.y1);
	...
	if (sd_root == RAT_TF)
	{
		...
		eLIBs_strcpy(disk_text, "TF");
		GUI_DispStringInRect(disk_text, &str_rect, GUI_TA_VCENTER);			
	}
	
	//usb

	str_rect.x0 = headbar_ui->usb_rect.x;
	str_rect.y0 = headbar_ui->usb_rect.y;
	str_rect.x1 = str_rect.x0 + headbar_ui->usb_rect.width;
	str_rect.y1 = str_rect.y0 + headbar_ui->usb_rect.height;
	...
	if(usb_root == RAT_USB)
	{
		if(usb_root == cur_type)
			GUI_SetColor(GUI_WHITE);
		else
			GUI_SetColor(GUI_GRAY);
		
		eLIBs_strcpy(disk_text, "USB");
		GUI_DispStringInRect(disk_text, &str_rect, GUI_TA_VCENTER);			
	}
...
//Headbar显示当前曲目
__s32 DrawMusicCurTotal(void)
{	
	music_headbar_ui_t *headbar_ui;
	GUI_RECT str_rect;
	char index_text[64], tmp_text[24];
	__u32 cur_type, root_type;
	__u32 sd_root =0;
	__u32 usb_root = 0;
	...
	//Tip;
	get_menu_text(STRING_MUSIC_PLAY_CUR_INDEX, index_text, sizeof(index_text));
	//Space;
	eLIBs_strcat(index_text, " ");
	//Cur Digital;
    eLIBs_int2str_dec(cur_song_num, tmp_text);
	eLIBs_strcat(index_text, tmp_text);
	//Space;
	eLIBs_strcat(index_text, "/");
	//Total Digital;
	...
	str_rect.x0 = headbar_ui->cur_rect.x;
	str_rect.y0 = headbar_ui->cur_rect.y;
	str_rect.x1 = str_rect.x0 + headbar_ui->cur_rect.width;
	str_rect.y1 = str_rect.y0 + headbar_ui->cur_rect.height;

	GUI_ClearRect(str_rect.x0, str_rect.y0, str_rect.x1, str_rect.y1);

	GUI_UC_SetEncodeUTF8();
	GUI_SetColor(GUI_WHITE);
	GUI_DispStringInRect(index_text, &str_rect, GUI_TA_VCENTER |GUI_TA_LEFT);			
...
{	
	...
	//if(dsk_volume_is_on() == EPDK_TRUE)  //cai++ for 是否静音
	//{
		//__msg("volume_on.....\n");
	...
	dsk_power_get_voltage_level(&(player_ctrl->battery_status),&is_full);		
#else //带PMU
	dsk_power_get_battery_level(&(player_ctrl->battery_status),&is_full);
#endif
	
	DrawMusicBatteryStatus(player_ctrl->battery_status, is_full);
...
__s32 CleanMusicButtonHintText(void)
{
	...
	GUI_ClearRect(text_rect.x0-20, text_rect.y0, text_rect.x1, text_rect.y1);	
	return EPDK_OK;
}
