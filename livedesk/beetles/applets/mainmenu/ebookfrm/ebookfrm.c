...
************************************************************************************************************************
*/
#include "ebookfrm_private.h"


static __s32 ebookfrm_on_key(__gui_msg_t *msg)
{		
	ebookfrm_attr_t*	frmwin_attr;

	frmwin_attr = (ebookfrm_attr_t *)GUI_WinGetAttr(msg->h_deswin);
...
static __s32 ebookfrm_on_paint(__gui_msg_t *msg,__u32 winsta)
{
	H_LYR			layer;
	RECT			rect;
	__pos_t		 	offset;	
...
static __s32 cb_ebookfrm_frm(__gui_msg_t *msg)
{	
	__msg("msg->id=%d\n",msg->id);
	switch(msg->id)
	{
		...
		{
			ebookfrm_on_paint(msg,MAINMENU_WINSTA_FCS);
		...
		{	
			ebookfrm_on_paint(msg,MAINMENU_WINSTA_UNFCS);
			break;
		}
...
H_WIN ebookfrm_frm_create(H_WIN parent, H_WIN layer,__u16 fid,__u32 lyr_type)
{
	__u32 						i;
	RECT                    				rect;
	H_WIN 						h_win;
	...
	////para
	frmwin_attr = (ebookfrm_attr_t *)esMEMS_Malloc(0, sizeof(ebookfrm_attr_t));
	if(frmwin_attr == NULL)
	{
		__wrn("malloc is error! \n");
		return NULL;
	}
	eLIBs_memset(frmwin_attr, 0, sizeof(ebookfrm_attr_t));
	...
	frame_info.FrameWinProc     = cb_ebookfrm_frm;
