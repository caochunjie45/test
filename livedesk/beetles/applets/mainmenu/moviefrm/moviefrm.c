...
************************************************************************************************************************
*/
#include "moviefrm_private.h"


static __s32 moviefrm_on_key(__gui_msg_t *msg)
{		
	moviefrm_attr_t*	frmwin_attr;

	frmwin_attr = (moviefrm_attr_t *)GUI_WinGetAttr(msg->h_deswin);
...
static __s32 moviefrm_on_paint(__gui_msg_t *msg,__u32 winsta)
{
	H_LYR			layer;
	RECT			rect;
	__pos_t		 	offset;	
	...
	
	{
		...
		{
			eDbug(" draw focus picture ,rect.x + offset.x is %d,rect.y + offset.y is %d \n",rect.x + offset.x ,rect.y + offset.y);
...
static __s32 cb_moviefrm_frm(__gui_msg_t *msg)
{	
	__msg("msg->id=%d\n",msg->id);
	switch(msg->id)
	{
		...
		{
			moviefrm_on_paint(msg,MAINMENU_WINSTA_FCS);
		...
		{	
			moviefrm_on_paint(msg,MAINMENU_WINSTA_UNFCS);
			break;
		}
...
H_WIN moviefrm_frm_create(H_WIN parent, H_WIN layer,__u16 fid,__u32 lyr_type)
{
	__u32 						i;
	RECT                    				rect;
	H_WIN 						h_win;
	...
	////para
	frmwin_attr = (moviefrm_attr_t *)esMEMS_Malloc(0, sizeof(moviefrm_attr_t));
	if(frmwin_attr == NULL)
	{
		__wrn("malloc is error! \n");
		return NULL;
	}
	eLIBs_memset(frmwin_attr, 0, sizeof(moviefrm_attr_t));
	...
	frame_info.FrameWinProc     = cb_moviefrm_frm;
