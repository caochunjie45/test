...
#define IOCTRL_DIALOG_ID				2004
#define IOECHO_DIALOG_ID				2005
#define FW_UPDATE_DIALOG_ID 			2006

#define LOW_POWER_TOUT					500
#define POWER_OFF_TOUT					500
#define IO_ECHO_TOUT					250
	...
	switch( p_scene->cur_dialog)
	{
		...
		{		
			...
			if( p_scene->io_ctrl)
			{
				...
				*/
				//////////////
				p_scene->cur_dialog = IOCTRL_DIALOG;
				//GUI_SetTimer(msg->h_deswin, LOW_POWER_DIALOG_TIME_ID, LOW_POWER_TOUT , NULL);	// 低电对话框存在时间计数
				scene_dialog_create(p_scene, STRING_LINEIN, IOCTRL_DIALOG_ID);				
			} 
