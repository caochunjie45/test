...
void dsk_keytone_send(void)
{
	ES_FILE *plife;
	__u8 *pbuf;

	plife = keytone->f_audiodev;
			
	pbuf = (__u8 *)esMEMS_Palloc(25, MEMS_PALLOC_MODE_BND_NONE | MEMS_PALLOC_MODE_BNK_NONE);
	if( !pbuf )
	{
		__wrn(" tonebuf malloc error \n");
		return ;
	}
		
	eLIBs_memset(pbuf, 0,1024*25);

	eLIBs_fwrite(pbuf, 1, 25*1024, plife);
			
	esMEMS_Pfree((void *)pbuf, 25);
