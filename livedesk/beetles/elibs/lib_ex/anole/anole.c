...
__bool anole_check_pic(__u32 index)
{
	...
	if(esMODS_MIoctrl(mod_willow, WILLOW_CMD_CHECK_IMG, 0, &param) == EPDK_FAIL)
	{
		__wrn("%d :the file format is not supported!\n", index);
		__msg("the file format is not supported!\n");
		__wait_willow_finish();
		return EPDK_FALSE;
	}

	__wrn("%d :the file format is supported!\n", index);
	return EPDK_TRUE;
}
#endif
...
**********************************************************************************************************************
*/
#define ANOLE_DEAD_LOOP_HERE    //__here__
...
{	
	...
	{		 
		...
		{	
			...
			if (orign_delay != 1)
			{
				__u32 count = (orign_delay - 1)*100/ANOLE_AUTO_PLAY_THREAD_DELAY_TIME;	//延时多次
				while( count )
				{
...
{	
	...
	__msg("anole_core.scale_level = %d, level = %d\n", anole_core.scale_level, level);
    if(level < -3 || level > 3)
    {
...
{	
	...
	__inf("zoom from %d ", anole_core.scale_level);	
