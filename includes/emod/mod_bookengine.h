...
typedef struct bookenginetocstru                /*用于保存目录的数据结构*/
{
	char			        title[128];         /*用于保存目录的title信息*/
	EBOOKTAG			    booktag;            /*用于保存对应该title的书签信息*/
	void                   *next;               /*下一条目录条目的指针*/
	int						stage;				/*当前目录的等级*/
}BOOKENGINETOCSTRU;
...
typedef enum
{
	EBOOK_LEFTTOP,              //左上角
	EBOOK_RIGHTTOP,             //右上角    
	EBOOK_LEFTBOTTOM,           //左下角
	EBOOK_RIGHTBOTTOM,          //右下角
	EBOOK_PT_UNKNOWN,           //未知
}EBOOK_STARTPT;

typedef struct ebook_tag_rendcore
{
	EBOOK_RECT2D rect;			//区域
	EBOOK_STARTPT pt;				//开始方式
	EBOOK_POINT2D currentpoint;	//当前点击坐标
	void* frontpagebuf;		//首页buf
	void* backpagebuf;		//次页buf
	void* resultbuf;		//结果buf,用于显示
