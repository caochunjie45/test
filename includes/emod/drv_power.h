...
{
    __u8    dcin_valid;                 //外部电源是否存在？1-存在；0-不存在
    __u8    charge_status;              //充电状态，1-正在充电；0-不在充电
    __u8    battery_exist;              //电池是否存在，1-存在；0-不存在
    __u8    battery_status;             //0: 充电  1：放电
    __u16   rest_vol;                   //剩余电池电量，百分比
    __u16   rest_time;                  //剩余电池时间或者剩余充电时间
    __u16   bat_vol;					//电池实时电压值？单位-伏
    __u16   bat_crnt;					//电池实时充放电电流值？单位-毫安
    __u16   bat_cap;					//电池实际容量，单位-毫安时
    __u16   bat_rdc;					//电池通路直流阻抗-毫欧
    __u8    dcin_not_enough;            //表示 DCIN 负载能力不足
    __u16   bat_vol_correct;			//电池校正后电压，单位-伏
...
{
    __u32   charge_status;              //当前状态，bit2:DCIN是否存在；  bit1:电池存在；  bit0: 充电状态，1为充电，0为没有充电
                                        //其它状态，意义保留
    __u32   rest_vol;                   //剩余电量，百分比
    __u32   rest_time;                  //剩余时间，分钟数
    __u32   bat_cap;                    //电池容量？单位-毫安时
    __u32   bat_power;                  //电池瞬态功率，单位-毫瓦
...
{
    __u16   dcin_exist          :2;//DCIN是否插入
    __u16   dcin_voladc_enable  :2;//DCIN电压ADC是否使能？
    __u16   dcin_crntadc_enable :2;//DCIN电流ADC是否使能？
    __u16   reserved            :10;
    __u16   adc_freq;               //ADC采样频率？
    __u16   dcin_vol;               //DCIN 实时电压值？单位-伏
    __u16   dcin_crnt;              //DCIN实时电流值？单位-毫安
...
*                                   关于PWM设置
*
*   和控制器状态有关的命令有  DRV_POWER_CMD_PWM_SET
*
*   这个结构体用于设置PWM频率和占空比，频率单位为Hz，占空比为1%
*
*   PWM频率最低能到35Hz，最高到1MHz，在1MHz时，占空比只有50%可设置
*  PWM频率低于17.576KHz，占空比分辨率才能达到1%精度
*
*****************************************************************************************
*/
typedef struct  __DRV_POWER_PWM
{
    __u32   frequency;                //PWM的频率，单位为Hz
    __u16   duty;                     //PWM的占空比单位为1%

}__drv_power_pwm_t;


#define POWER_EPT_1                 0x1f00  //AXP18x,对应 PIN-41（DRV1-CDOUTPL）AXP19x，对应GPIO0
#define POWER_EPT_2                 0x1f01	//AXP18x,对应 PIN-42（DRV2-CDOUTNL）AXP19x，对应GPIO1
#define POWER_EPT_3                 0x1f02	//AXP18x,对应 PIN-45（DRV3-CDOUTPR）AXP19x，对应GPIO2
#define POWER_EPT_4                 0x1f03	//AXP18x,对应 PIN-46（DRV4-CDOUTNR）AXP19x，对应GPIO3
#define POWER_EPT_5                 0x1f04	//对应AXP19x，GPIO4
#define POWER_EPT_6                 0x1f05	//对应AXP19x，GPIO5（N_RSTO）
#define POWER_EPT_7                 0x1f06	//对应AXP19x，GPIO6（PWREN）
#define POWER_EPT_8                 0x1f07	//对应AXP19x，GPIO7（SYSEN）

#define POWER_EPT_MOS_N             (0x00 << 2)//对应AXP18x IO拉低；AXP19x IO拉低
#define POWER_EPT_MOS_P             (0x01 << 2)//对应AXP18x IO拉高；AXP19x IO拉高
#define POWER_EPT_MOS_NOTCARE       (0x02 << 2)

#define POWER_EPT_GATE_OFF          (0x00 << 0)//对于AXP18x，关闭则不能拉高、拉低
#define POWER_EPT_GATE_ON           (0x01 << 0)//对于AXP18x，打开才能拉高、拉低
...
#define POWER_VOL_ON                0x01				//打开输出电压
#define POWER_VOL_OFF               0x00        //关闭输出电压

#define POWER_OPEN                	0x01        // GPIO1 输出高电平或者打开外部充充 对应DRV_POWRE_CMD_BL_CTRL、DRV_POWER_CMD_EXTCHG_CTRL
#define POWER_CLOSE               	0x00        // GPIO1 输出低电平或者关闭外部充充 对应DRV_POWRE_CMD_BL_CTRL、DRV_POWER_CMD_EXTCHG_CTRL

#define POWER_VBUS_CUR_900          0x01        //VBUS 限流电流900mA
#define POWER_VBUS_CUR_500          0x02        //VBUS 限流电流500mA
#define POWER_VBUS_CUR_100          0x03        //VBUS 限流电流100mA
#define POWER_VBUS_CUR_NOT_LIMIT    0x00        //VBUS 不限流电
...
#define POWERON_TIME_128MS          (0<<6)      //设置PowerON按键开机时间为128ms
#define POWERON_TIME_512MS          (1<<6)	    //设置PowerON按键开机时间为512ms或者3s
#define POWERON_TIME_1S             (2<<6)	    //设置PowerON按键开机时间为1s
#define POWERON_TIME_2S             (3<<6)	    //设置PowerON按键开机时间为2s
#define LONGKEY_TIME_1000MS			(0<<4)      //设置长按键时间为1s
#define LONGKEY_TIME_1500MS			(1<<4)	    //设置长按键时间为1.5s
#define LONGKEY_TIME_2000MS			(2<<4)	    //设置长按键时间为2s
#define LONGKEY_TIME_2500MS			(3<<4)	    //设置长按键时间为2.5s
#define PWM1												0x01
#define PWM2												0x02
...
typedef enum __DRV_POWER_CMD_GROUP
{
    ...
    DRV_POWER_CMD_BASIC_STATUS,                 //P1基本状态，对应__drv_power_basic_status_t
    ...
    DRV_POWER_CMD_PWREPT_GET,                   //IO状态读取
    DRV_POWER_CMD_PWREPT_SET,                   //IO状态设置
    DRV_POWER_CMD_BATADC_GET,                   //电池采样状态读取，对应__drv_power_battery_status_t1
    DRV_POWER_CMD_BATADC_SET,                   //电池采样开关设置，对应__drv_power_battery_status_t1
    DRV_POWER_CMD_DCINADC_GET,                  //DCIN采样状态读取，对应__drv_power_dcin_status_t
    DRV_POWER_CMD_DCINADC_SET,                  //DCIN采样开关设置，对应__drv_power_dcin_status_t
    DRV_POWER_CMD_CHARGE_GET,                   //充电状态读取，对应__drv_power_charge_status_t
    DRV_POWER_CMD_CHARGE_SET,                   //充电状态设置，对应__drv_power_charge_status_t
    DRV_POWER_CMD_IPS_GET,                      //IPS数据读取，对应__drv_power_ips_status_t
    DRV_POWER_CMD_IPS_SET,                      //IPS状态设置，对应__drv_power_ips_status_t
    DRV_POWER_CMD_BATSTATUS_TABLE_GET,          //状态表、获取电池电压、充放电电流、电池充放电剩余时间，对应__drv_power_battery_status_t2
    DRV_POWER_CMD_GET_BAT_RDC,                  //获取电池通路直流阻抗，单位毫欧
    DRV_POWER_CMD_GET_DCIN,                     //获取DCIN通路是否有效，是否Enable，充电状态？对应__drv_power_battery_status_t3
    DRV_POWER_CMD_GET_FUELGUAGE,                //读取电池电量，时间，状态,新加cmd，对应__drv_power_fuelguage_t
    ...
    DRV_POWER_CMD_ONOFFTIME_GET,                //获取AXP19x Timer信息，
    DRV_POWER_CMD_ONOFFTIME_SET,                //设置AXP19x Timer信息，
    ...
    DRV_POWER_CMD_SET_POWERON_TIME,             //设置开机时间 128ms/512ms/1s/2s,在aux传参
    DRV_POWER_CMD_SET_LONGKEY_TIME,             //设置长按键时间 1s/1.5s/2s/2.5s,在aux传参
    DRV_POWRE_CMD_BL_CTRL,                      //外部GPIO作为PWM控制背光开关控制  aux =POWER_OPEN为打开 aux =POWER_CLOSE 为关闭
    DRV_POWER_CMD_EXTCHG_CTRL,					//外部充电开关控制  aux =POWER_OPEN为打开 aux =POWER_CLOSE 为关闭
    DRV_POWER_CMD_SET_VBUS_CURR,				//设置USB限流电流 --100mA、500mA、900mA、不限流，该功能仅AXP20x和AXP19x有些区别，在pbuffer中传递参数
    DRV_POWER_CMD_SET_EXTCHG_CURR,				//设置外部充电电流，单位mA,只能设置300mA、400mA、500mA、600mA、700mA、800mA、900mA、1000mA，在pbuffer中传递参数
    DRV_POWER_CMD_PWM_SET,						//设置PWM的通道，PWM的频率以及PWM的占空比，其中
	
	DRV_POWER_CMD_SET_LOW_POWER,               //设置低电标志
