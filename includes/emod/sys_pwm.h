...
{
	__u32    sdmmc0_status;		/*进入stanby时sd0有没插入0:没有，1:插入*/
	__u32    usb0_status;		/*进入standby时usb0状态，0:无连接,1:device连PC,2:host连接u盘*/		
...
typedef struct __SYS_PWM_PARA
{
	...
	__u32       HKeyVal;			//standby 中ADC检查到的值  20140417 hp
	__enter_standby_status_t enter_standby_status;	/*进入stanby时候状态*/
} __sys_pwm_para_t;
